 ...
 *                 VarNode
 *********************************************/
void VarNode::accept(Visitor& visitor) { visitor.doVarNode(this); }

void VarRefNode::accept(Visitor& visitor) { visitor.doVarRefNode(this); }
...
void OpNode::accept(Visitor& visitor) {
  ...
  if (m_leftOpnd) m_leftOpnd->accept(visitor);
  if (m_rightOpnd) m_rightOpnd->accept(visitor);
...
string OpNode::toString() {
  ...
  retStr.append("(");
  if (hasLeftOpnd()) retStr.append(m_leftOpnd->toString());

  retStr.append(" ");
  retStr.append(getOpStr());
  retStr.append(" ");

  if (hasRightOpnd()) retStr.append(m_rightOpnd->toString());
  retStr.append(")");
  return retStr;
}
...
void NewNode::accept(Visitor& visitor) { visitor.doNewNode(this); }
