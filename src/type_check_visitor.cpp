...
void TypeCheckVisitor::doPrintNode( PrintNode* node ) {
	...
	if( ! node->canPrint( dtypeNode ) ){
        ...
        LOG4CXX_DEBUG( ms_logger, 
                       "expression \"" << node->m_exp->toString()
                                       << "\" with type \"" 
...
void TypeCheckVisitor::doVarRefNode( VarRefNode* node ){
	...
	if( NULL == var ){
        ...
        VarNotDeclareError error( node->getLineNum(), 
                                  node->getName() );
        emitError( &error );
...
void TypeCheckVisitor::doGlobalFuncNode( GlobalFuncNode* funcNode ){
    ...
    if( NULL != funcTmp ){
        ...
        LOG4CXX_DEBUG( ms_logger, "function \"" << funcNode->getName() << "\" redefined");
...
void TypeCheckVisitor::doCallNode( CallNode* node ) {
    ...
    if( node->getMemberFuncHint() ){
        ...
        LOG4CXX_DEBUG( ms_logger, "hint function \"" 
                                  << node->getFuncName() 
        ...
        if( node->getCallerName().size() == 0 ) {
            ...
            if( NULL == caller ) {
                ...
                if( NULL == classNode ){
                    ClassNotDefineError error( classType->getClassName(), node->getLineNum());
                }else{
