...
bool IntTypeNode::canCast( DataTypeNode* node ) {
    ...
    assert( node );
    if( isEqual(node) || node->getDataType() == TYPE_CHAR )
        return true;
    return false;
...
bool CharTypeNode::canCast( DataTypeNode* node ) {
    ...
    assert( node );
    if( isEqual(node) || node->getDataType() == TYPE_INT )
        return true;
    return false;
...
void ClassTypeNode::accept( Visitor& visitor ){
...
bool ClassTypeNode::isEqual( DataTypeNode* dataType ){
    ...
    if( NULL == dataType || dataType->getDataType() != TYPE_CLASS )
        return false;
    if( this == dataType )
        return true;
