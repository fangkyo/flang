...
void ScopeManager::pushFuncLevel( FuncNode* funcNode ){
    ...
    if( NULL == funcNode )
        return;
    LOG4CXX_DEBUG( ms_logger, "push function \"" 
                              << funcNode->getName() 
                              <<"\"" );
...
void ScopeManager::pushClassLevel( ClassNode* classNode ){
    ...
    if( NULL == classNode ) 
        return;
    LOG4CXX_DEBUG( ms_logger, "push class \"" 
                              << classNode->getName() 
                              << "\"" );
...
void ScopeManager::popLevel(){
    ...
    LOG4CXX_DEBUG( ms_logger, "pop level \"" 
                              << m_scopeStack.front().toString()
                              << "\"");
...
void ScopeManager::addVar( VarNode* varNode ) {
    ...
    if( NULL == varNode )
        return;
    LOG4CXX_DEBUG( ms_logger, "add var \"" 
                              << varNode->getName() 
                              << "\"" );
...
void ScopeManager::addClass(ClassNode* classNode) {
    ...
    if( NULL == classNode )
        return;
    LOG4CXX_DEBUG( ms_logger, "add class \"" 
                              << classNode->getName() 
                              << "\"" );
...
VarNode* ScopeManager::testVarConflict( const string& varName ){
    ...
    if( varNode ) {
        LOG4CXX_DEBUG( ms_logger, "testVarConflict : var \"" << varName << "\" conflicted" );
    }else {
        LOG4CXX_DEBUG( ms_logger, "testVarConflict : var \"" << varName << "\" not conflicted" );
    }
...
FuncNode* ScopeManager::findFunc( const string& funcName, vector<DataTypeNode*>& paramsType ) {
...
FuncNode* ScopeManager::findMemberFunc( const string& funcName, vector<DataTypeNode*>& paramsType ) {
...
int ScopeManager::findFuncScope( FuncNode* &funcNode ) {
    ...
    while( iter != m_scopeStack.end() ){
        ...
        if( iter->getScopeType() == SCOPE_FUNC ){
            LOG4CXX_DEBUG( ms_logger, "find function scope in scope stack at level "<< level );
            funcNode = iter->getFuncNode();
            return level;
        }
        ++level;
...
int ScopeManager::findClassScope( ClassNode* &classNode ) {
    ...
    while( iter != m_scopeStack.end() ){
        if( iter->getScopeType() == SCOPE_CLASS ){
            LOG4CXX_DEBUG( ms_logger, "find class scope in scope stack at level "<< level );
