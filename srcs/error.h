...
class VarRedeclareError : public Error {
 ...
 public:
  VarRedeclareError(VarNode* varNode) : m_varNode(varNode) {
    assert(m_varNode);
  }
  string toString();
};
...
class BreakWithNoWhileError : public Error {
 ...
 public:
  BreakWithNoWhileError(BreakNode* breakNode) : m_breakNode(breakNode) {
    assert(m_breakNode);
  }
  string toString();
};
...
class ReturnTypeError : public Error {
 ...
 public:
  ReturnTypeError(FuncNode* func, ReturnNode* ret) : m_func(func), m_ret(ret) {
    assert(m_func);
    assert(m_ret);
  }
...
class SingleReturnError : public Error {
 ...
 public:
  SingleReturnError(ReturnNode* ret) : m_ret(ret) { assert(m_ret); }
...
class InheritSelfError : public Error {
 ...
 public:
  InheritSelfError(ClassNode* clazz) : m_class(clazz) { assert(m_class); }
