...
class SymbolTest : public ::testing::Test {
 public:
  void SetUp() override {
    var_.reset(new VariableSymbol("a"));
    func_.reset(new FunctionSymbol("b"));
    class_.reset(new ClassSymbol("c"));
  }
 protected:
...
TEST_F(SymbolTest, testFunctionSymbol) {
  ...
  VariableSymbol* p1 = new VariableSymbol("p1");
  p1->setDataType(DataTypeFactory::getInt32Type());
  func_->addParameter(p1);
  VariableSymbol* p2 = new VariableSymbol("p2");
  ...
  EXPECT_EQ(func_.get(), scope->lookup(func_->getName()));
