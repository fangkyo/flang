...
class ExpNode : public SyntaxNode {
    ...
    public:
        DataTypeNode* m_dataTypeNode; //数据类型字段，该字段不需要在析构函数中释放，而是由外部的类型管理工具维护

        ExpNode() {  m_dataTypeNode = UndefTypeNode::getInstance(); }

        void setDataTypeNode( DataTypeNode* dtypeNode )  { m_dataTypeNode = dtypeNode; }

        DataTypeNode* getDataTypeNode()    { return m_dataTypeNode; }
...
class StringNode : public ExpNode {
    ...
    public:
        ...
        StringNode( const string& str ) : m_val( str ) { setDataTypeNode( StringTypeNode::getInstance() ); } 

        StringNode( const char* str ) : m_val( str ) { setDataTypeNode( StringTypeNode::getInstance() ); }
...
class BoolNode : public ExpNode {
    public:
        ...
        BoolNode( bool val ) : m_val( val ) { setDataTypeNode( BoolTypeNode::getInstance() );}
...
class CharNode : public ExpNode {
    public:
        ...
        CharNode( char val ) : m_val( val ) { setDataTypeNode( CharTypeNode::getInstance() ); } 
...
class OpNode : public ExpNode {
    public:
        ...
        DataTypeNode* getRightDataTypeNode() { return m_rightOpnd->getDataTypeNode(); }
};
...
class AssignNode : public ExpNode{
    public:
        ...
        AssignNode( VarNode* var, ExpNode* exp = NULL ) : m_var( var ), m_exp( exp ) { assert( m_var);}
...
class NewNode : public ExpNode {
    ...
    public:
        NewNode( const string& className ) 
        : m_className( className ), m_class(NULL) {
        }
